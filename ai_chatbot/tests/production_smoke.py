"""Production smoke assertions executed after install + migrate."""
...
def run() -> dict:
	"""Verify install contracts, row isolation, and user-bound confirmations."""
	...
	confirmation_id = f"ci-{uuid.uuid4()}"
	...
	try:
		...
		foreign_messages = get_conversation_messages(conv_b.name)
		assert not foreign_messages.get("success"), "conversation API IDOR protection failed"

		try:
			get_conversation_history(conv_b.name)
			raise AssertionError("history IDOR protection failed")
		...
		payload = {
			...
			"action": "create",
			"doctype": "ToDo",
			"values": {"description": "CI"},
		}
		_store_pending_confirmation(confirmation_id, payload)
		loaded = load_pending_confirmation(confirmation_id)
		assert loaded and loaded.get("confirmation_id") == confirmation_id

		frappe.set_user(user_b)
		try:
			load_pending_confirmation(confirmation_id)
			raise AssertionError("confirmation token ownership protection failed")
		except frappe.PermissionError:
			pass
