...
def create_conversation(title: str, ai_provider: str = "OpenAI", foundry_agent: str | None = None) -> dict:
	...
	foundry_agent: name of a Foundry Agent record (see api/foundry_agents.py),
	only meaningful when ai_provider == "Azure AI Foundry Agent". Ignored
...
def get_conversations(limit: int = 20) -> dict:
	...
	try:
		limit = bounded_int(limit, default=20, maximum=100)
...
def send_message(
	...
	try:
		...
		# Get AI provider
		provider = get_ai_provider(conversation.ai_provider, conversation=conversation)
